  ...
  public async executeStep4Publish(options?: {
    ...
    for (const raw of itemsToPublish) {
      ...
      const storeProduct: any = {
        ...
        stockAvailable: raw.stock > 0 ? raw.stock : 0,
        status: raw.stock > 0 ? 'AVAILABLE' : 'OUT_OF_STOCK',
        isAvailable: raw.stock > 0,
        outOfStockReason: raw.stock <= 0 ? 'Sản phẩm đã bán hết tại shop API nguồn' : undefined,
        tags: ['G2UP API', 'Cyborg Direct', classification.label, 'Bảo Hành 10D', ...(raw.stock <= 0 ? ['OUT_OF_STOCK'] : [])],
