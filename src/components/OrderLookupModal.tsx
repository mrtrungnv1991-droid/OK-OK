            ...
            <div className="space-y-4 pt-2">
              {foundOrder ? (
                <div className="p-4 rounded-xl bg-slate-900/70 border border-cyan-500/40 space-y-4">
                  ...
                  {/* Digital Delivery Output Section (Account, Key, Link, Giftcard) */}
                  {foundOrder.deliveredKey && (
                    <WebDeliveryOutput
                      branch={foundOrder.deliveryBranch}
                      rawKey={foundOrder.deliveredKey}
                      accountCredentials={foundOrder.deliveredData?.accountCredentials}
                      inviteLink={foundOrder.deliveredData?.inviteLink}
                      giftCardInfo={
                        foundOrder.deliveredData?.giftCardInfo ||
                        (foundOrder.pinCode
                          ? { cardNumber: foundOrder.deliveredKey, pinCode: foundOrder.pinCode }
                          : undefined)
                      }
