            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-amber-950/20 border border-amber-500/30 space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-amber-300">RESELLER API — CHƯA TRIỂN KHAI</span>
                </div>
                {/* CYBERPOOL FIX (#14 frontend audit): trước đây tab này hiển thị
                    "live secret key" GIẢ (cp_live_sec_... hardcode, nút Refresh
                    sinh chuỗi Math.random client-side) không xác thực điều gì —
                    reseller tin rằng có API thật. Trung thực: API reseller chưa
                    có backend; sẽ mở khi hệ thống API key + webhook hoàn thiện. */}
                <p className="text-[11px] text-slate-300 leading-relaxed">
                  Hệ thống Reseller API (khóa bí mật + webhook callback) đang trong giai đoạn phát triển và
                  <strong className="text-amber-300"> chưa được kích hoạt</strong>. Hiện tại bạn vẫn có thể
                  kiếm hoa hồng đầy đủ qua <strong className="text-cyan-300">mã giới thiệu</strong> ở tab Tổng quan —
                  chia sẻ liên kết và nhận hoa hồng cho mỗi đơn hàng thành công.
                </p>
                <p className="text-[11px] text-slate-400">
                  Khi API reseller ra mắt, khóa bí mật sẽ được cấp phát và quản lý từ máy chủ (không hiển thị sẵn),
                  kèm tài liệu webhook chính thức. Vui lòng quay lại sau hoặc liên hệ hỗ trợ để đăng ký sớm.
                </p>
              </div>
            </div>
          )}
